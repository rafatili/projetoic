classdef Cpaciente < handle 
    ...
    methods 
        function obj = Cpaciente(prop1)%,prop2,prop3)
            if nargin == 1
                
                obj.arquivo_dat = prop1;               
                %obj.empresa = dlmread(prop1,'\t',[3 2 3 2]);
                obj.num_canais = max(dlmread(prop1,'\t',[15 0 36 0]));
                obj.maxima = dlmread(prop1,'\t',[3 1 3 1]);
                obj.interphase_gap = mean(dlmread(prop1,'\t',[4 1 4 1]))*1e-6;
                obj.largura_pulso1 = mean(dlmread(prop1,'\t',[15 7 36 7]))*1e-6;
                obj.largura_pulso2 = mean(dlmread(prop1,'\t',[15 7 36 7]))*1e-6; 
                obj.T_SPL = dlmread(prop1,'\t',[8 1 8 1]);
                obj.C_SPL = dlmread(prop1,'\t',[9 1 9 1]);
                obj.T_Level = dlmread(prop1,'\t',[15 3 36 3]);
                obj.C_Level = dlmread(prop1,'\t',[15 4 36 4]);
                obj.inf_freq = dlmread(prop1,'\t',[15 9 36 9]);
                obj.sup_freq = dlmread(prop1,'\t',[15 10 36 10]);
